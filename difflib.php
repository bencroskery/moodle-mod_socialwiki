...
function socialwiki_diff_merge(&$k, &$t, $i, &$e, $p, &$candidates) {
    ...
    while (true) {
        ...
        // Binary search in $k from $r to $t.
        $min = $r;
        $max = $t + 1;
        ...
        while (true) {
            ...
            if ($candidates[$k[$try]]->b >= $j) {
                ...
                $min = $try + 1;
            } else { // The $try is less and $try+1 is more.
                $s = $try;
                break;
            }
            if ($max <= $min) {
...
class socialwiki_diffcontent_line {
    ...
    public function __construct($data, $linepos) {
        ...
        while (true) {
            ...
            // And find the next space after that.
...
class socialwiki_diffcontent_changes {
    ...
    public function __construct($diff, $count2) {
        ...
        foreach ($diff as $index1 => $index2) {
            ...
            if ($index2 === 0 && !in_array($index1, $this->deletes)) {
                ...
                if ($index2 !== 0) {
                    ...
                    if ($lastrange !== -1) {
                        $this->changes[$lastrange]->file2count = $index2 - $this->changes[$lastrange]->file2start;
                        $lastrange = -1;
                    }
                }
            }
        ...
        if ($lastrange !== -1) {
            $this->changes[$lastrange]->file2count = $count2 - $this->changes[$lastrange]->file2start + 1;
        }
    }
